...
from organizational_memory.models import OpenLoop, OpenLoopStatus


def test_open_loop_defaults() -> None:
    loop = OpenLoop(question="Who owns billing?")
    assert loop.status is OpenLoopStatus.OPEN
...
def test_open_loop_full_construction() -> None:
    ...
    assert loop.status is OpenLoopStatus.RESOLVED
